  ...
  class Dashboard {
    ...
    addReportToUI(report, replaceExisting = false) {
      ...
      const imageUrl = report.image_path
        ? `/GreenBin/uploads/${report.image_path}?t=${new Date().getTime()}`
        : "/GreenBin/frontend/img/default-report.png";

      let existingCard = this.reportsList.querySelector(
      ...
      if (replaceExisting && existingCard) {
        ...
        const metaInfo = existingCard.querySelector(
          ".report-meta, .text-sm.text-gray-500"
        );
        ...
        if (description)
          description.innerHTML = this.nl2br(
            this.escapeHtml(report.description || "")
          );
        if (metaInfo) {
          metaInfo.innerHTML = `
            <span>Status: <strong class="status-${report.status
              ?.toLowerCase()
              .replace(" ", "-")}">${this.escapeHtml(
            report.status || ""
          )}</strong></span>
      ...
      reportCard.className =
        "report-card bg-white border rounded-lg p-4 shadow-sm hover:shadow-md transition-shadow";
      ...
      reportCard.innerHTML = `
        ...
        <div class="report-content">
          <h3 class="report-title font-semibold text-lg mb-2 text-gray-800">${this.escapeHtml(
            report.title || ""
          )}</h3>
          <p class="report-description text-gray-600 mb-3 text-sm leading-relaxed">${this.nl2br(
            this.escapeHtml(report.description || "")
          )}
          </p>
    ...
    async loadUserReports() {
      ...
      try {
        ...
        if (data.success && data.reports && data.reports.length > 0) {
          ...
          // Remove reports from UI that are no longer present
          for (const existingId of this.currentReportsMap.keys()) {
            if (!fetchedReportsMap.has(existingId)) {
              const cardToRemove = this.reportsList.querySelector(
          ...
          // Add new or update existing reports
          for (const report of data.reports) {
            const reportId = report.report_id;
            const existingReport = this.currentReportsMap.get(reportId);

            if (!existingReport) {
              // New report
              this.addReportToUI(report);
              this.currentReportsMap.set(reportId, report);
            } else {
              // Existing report, check if it has changed
              const oldHash = this.hashReport(existingReport);
    ...
    async deleteReport(reportId) {
      ...
      try {
        ...
        if (result.success) {
          const card = this.reportsList.querySelector(
          ...
          this.showToast(
            "Failed to delete report: " + (result.message || "Unknown error"),
            "error"
          );
        }
      } catch (err) {
    ...
    tryGeolocation() {
      ...
      const locationPermissionMessage = document.getElementById(
      ...
      if (!navigator.geolocation) {
        locationPermissionMessage.textContent =
      ...
      locationPermissionMessage.textContent =
        "Please allow location access to automatically detect your location. You can change it later.";
      if (!storedLocation) {
        // Only show "Getting location..." toast if no cached location
    ...
    async handleFormSubmit(e) {
      ...
      try {
        const res = await fetch(
          this.form.action || "/GreenBin/backend/reportSubmit.php",
          {
            method: "POST",
            body: formData,
          }
        ...
        if (data.success) {
          ...
          if (data.report) {
            ...
            // Store the full report object, not just the hash
            this.currentReportsMap.set(data.report.report_id, data.report);
          }
    ...
    updateFileInputUI() {
      const fileName =
        this.fileInput.files[0]?.name || "Click to upload or drag and drop";
    ...
    openEditModal(report) {
      ...
      if (existingImageField) existingImageField.value = report.image_path || "";
    }
    ...
    getReportDataFromMap(reportId) {
      // The map now stores the full report object.
      return this.currentReportsMap.get(parseInt(reportId, 10)) || null;
    }
    async fetchReportAndOpenModal(reportId) {
      try {
        const res = await fetch(
          `/GreenBin/backend/getReport.php?id=${reportId}`
        );
        ...
        if (data.success && data.report) {
          ...
          this.showToast(
            data.message || "Could not find report details.",
            "error"
          );
        }
      } catch (err) {
    ...
    async handleEditFormSubmit(e) {
      ...
      try {
        ...
        if (result.success) {
          this.closeEditModal();
          if (result.report) {
            this.addReportToUI(result.report, true); // Update the UI
            this.currentReportsMap.set(result.report.report_id, result.report); // Store the full object
          } else {
            this.loadUserReports(); // Fallback to reload all
          }
          this.showToast(result.message || "Report updated successfully.");
        } else {
      ...
      try {
        ...
        if (data.success && data.stats) {
          ...
          document.getElementById("resolvedCount").textContent =
            (stats.resolvedCount || 0) + " resolved";
          document.getElementById("co2Reduction").textContent =
            parseFloat(stats.co2Reduction || 0).toFixed(1) + " kg";
