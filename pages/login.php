 ...
 * - Only allow users to continue to dashboard.
 * - If role is not "user", destroy session and redirect to login with an error.
 */
if (isset($_SESSION['user_id'])) {
  $role = $_SESSION['role'] ?? '';

  if ($role === 'user') {
    header("Location: /GreenBin/dashboard");
    exit;
  } else {
    session_destroy();
    header("Location: /GreenBin/login.php?error=unauthorized");
    exit;
  }
}
?>
...
<body class="relative min-h-screen bg-gray-200 bg-cover bg-center bg-no-repeat">
  ...
  <div class="flex justify-center items-center min-h-screen px-4 bg-overlay">
    <form action="/GreenBin/backend/login.php" method="POST"
      ...
      <!-- ✅ Dynamic Message (Error/Success) -->
      <div id="message" class="text-sm text-center mb-3 font-medium text-red-600" style="min-height: 1.5rem;">
        <?php if (isset($_GET['error']) && $_GET['error'] === 'unauthorized'): ?>
          ❌ Unauthorized Access: Admins cannot log in here.
      ...
      <div class="mb-4">
        <button type="submit"
          class="w-full bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-4 rounded-lg shadow-sm transition duration-300">
          Login
        </button>
      </div>
